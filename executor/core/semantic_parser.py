...
--------------------------------
Phase 2.10b.1 — deterministic parser with improved trip detection

Updates:
- RX_TRIP_TO now matches bare forms like "heading to Fort Worth" or "going to Chicago"
- All 2.10a.8 / 2.10b reasoning hooks retained
"""
...
RX_WHERE_AM_I_GOING = re.compile(
    r"(?i)\b(where\s+am\s+i\s+going|trip\s+destination|where\s+is\s+my\s+trip)\??$"
)

# ---------- Location updates ----------
RX_LIVE_IN = re.compile(
    r"(?i)\b(i\s+live\s+in|my\s+home\s+is\s+in)\s+(?P<city>.+)$"
)
RX_IM_IN = re.compile(
    r"(?i)\b(i'?m|i\s+am)\s+(?:currently\s+)?(?:in|at|staying\s+in|visiting)\s+(?P<city>.+)$"
)
...
RX_CHANGE_MY = re.compile(
    r"(?i)\bchange\s+my\s+(?P<key>.+?)\s+to\s+(?P<val>.+)$"
)

# ---------- Preferences / sentiments ----------
RX_I_LIKE = re.compile(
    r"(?i)\b(i\s+(?:really\s+)?(?:like|love|enjoy|am\s+into)\s+)(?P<item>.+)$"
)
RX_I_DISLIKE = re.compile(
...
RX_REFLECTIVE = re.compile(
    r"(?i)\b(why|how\s+come|what\s+makes|what\s+causes)\b.*\b(i|me|my)\b"
)
...
def _parse_single_clause(t: str, intimacy_level: int) -> List[Dict[str, Any]]:
    ...
    # Reflective / therapy-adjacent
    if RX_REFLECTIVE.search(tt):
        intents.append(
            _mk("reflective.question", None, None, None, 0.9,
                tone="reflective", intimacy=2)
        )
    ...
    if m:
        ...
        intents.append(
            _mk("fact.query", gmem.detect_domain_from_key(key),
                key, None, 0.85)
        )

    if RX_WHERE_AM_I_GOING.search(tt):
        intents.append(
            _mk("location.query", "location", "trip", None, 0.9, scope="trip")
        )

    if RX_WHERE_AM_I.search(tt):
        intents.append(
            _mk("location.query", "location", "current", None, 0.9, scope="current")
        )

    if RX_WHERE_DO_I_LIVE.search(tt):
        intents.append(
            _mk("location.query", "location", "home", None, 0.9, scope="home")
        )
    ...
    if m:
        intents.append(
            _mk("location.update", "location", "home",
                m.group("city").strip(), 0.95, scope="home")
        )

    m = RX_IM_IN.search(tt)
    if m:
        intents.append(
            _mk("location.update", "location", "current",
                m.group("city").strip(), 0.95, scope="current")
        )

    m = RX_TRIP_TO.search(tt)
    if m:
        intents.append(
            _mk("location.update", "location", "trip",
                m.group("city").strip(), 0.95, scope="trip")
        )
    ...
    if m:
        ...
        intents.append(
            _mk("fact.update", gmem.detect_domain_from_key(key),
                key, val, 0.92)
        )
    ...
    if m:
        ...
        intents.append(
            _mk("preference.statement", dom, item, None, 0.9,
...
def parse_message(text: str, intimacy_level: int = 0) -> List[Dict[str, Any]]:
    ...
    print(f"[ClauseSplit] {clauses}")  # optional debug
