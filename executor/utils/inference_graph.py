...
---------------------------------
Stores inferred preferences for contextual reasoning.
"""
...
def upsert_inferred_preference(domain: str, item: str, polarity: int, confidence: float):
    init_inference()
    conn = _connect(); c = conn.cursor()
    ts = _now()
    c.execute("DELETE FROM inferred_preferences WHERE domain=? AND item=?", (domain, item))
    c.execute("INSERT INTO inferred_preferences(domain,item,polarity,confidence,updated_at) VALUES(?,?,?,?,?)",
...
def list_inferred_preferences(domain: Optional[str] = None) -> List[Dict[str, Any]]:
    ...
    return [{"domain":r[0],"item":r[1],"polarity":r[2],"confidence":r[3],"updated_at":r[4]} for r in rows]