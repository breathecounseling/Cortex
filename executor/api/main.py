...
--------------------
Phase 2.13c — Goal Manager, Drift Awareness, and Goal Resume (API + chat)

Includes:
- /goals, /goals/close, /goals/resume endpoints
- goal.create / goal.close / goal.resume intents (via reasoner)
- drift nudges when an open goal becomes stale
- stable preference/location/fact handling
"""
...
def _session_id(request: Request, body: ChatBody) -> str:
    # Header takes precedence if present; otherwise use body or "default"
...
# ------------------------------------------------------------
# Inference refresh (preferences + relationships)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Helper — build model context (DB facts, vectors, etc.)
# ------------------------------------------------------------
def build_context_with_retrieval(query: str) -> list[dict[str, str]]:
    ...
    # Vector summaries + hierarchical recall
    try:
...
async def chat(body: ChatBody, request: Request) -> Dict[str, Any]:
    ...
    # Topic setter (explicit user switch)
    ...
    for p in parsed_intents:
        ...
        # --- Guard: direct conversational reply from reasoner (nudge/resume/create/close)
        if isinstance(intent, dict) and intent.get("reply") and intent.get("intent") in {
            "nudge", "goal.create", "goal.close", "goal.resume"
        }:
            replies.append(intent["reply"])
            # For goal create/close/resume we return immediately (single action responses feel best)
            if intent.get("intent") in {"goal.create", "goal.close", "goal.resume"}:
                final_reply = " ".join(r for r in replies if r).strip()
                add_turn("assistant", final_reply, session_id)
                return {"reply": final_reply}
            continue

        # --- Guard: reasoner may return {"reply": "..."} without "intent"
        ...
        # --- Goal / project request → planning + design context (unchanged)
        if intent.get("intent") in ("goal.statement", "project.request"):
            ...
            replies.append(f"{hint}. {q2}" if hint else q2)
            continue

    # ------------------------------------------------------------
    # Router / LLM fallback (unchanged)
    # ------------------------------------------------------------
    if not replies:
        try:
