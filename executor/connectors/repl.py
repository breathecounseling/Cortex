...
# ✅ Import OpenAIClient at module level so monkeypatch works in tests
...
def main() -> None:
    ...
    while True:
        ...
        # ✅ Use the module-level OpenAIClient reference (can be monkeypatched)
        client = OpenAIClient()
        try:
            data = router.route(user_text, session=SESSION)
        except Exception:
            # fallback to stub if router fails
            data = {"assistant_message": f"(stub) You said: {user_text}"}
